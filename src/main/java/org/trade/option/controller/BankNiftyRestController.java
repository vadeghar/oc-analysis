...
public class BankNiftyRestController {
    ...
    public BankNiftyRestController(SpotPriceService spotPriceService, BankNiftyService bankNiftyService) {
    ...
    public Map<String, Object> refresh(@PathVariable(required = false) String sortBy, @PathVariable(required = false) String orderBy) {
        ...
        List<BankNifty> niftyList = bankNiftyService.findAll(inputDay, sort);
    ...
    public Map<String, Object> refreshAnalysis(@PathVariable(required = false) String sortBy, @PathVariable(required = false) String orderBy) {
        ...
        if(orderBy != null)
            sort = orderBy.equals("ASC") ? sort.ascending() : sort.descending();
        List<String> insertedTimeList = bankNiftyService.getInsertedTimeList(inputDay, sort);
        List<BankNifty> todayData = bankNiftyService.findByUdatedAtSource(inputDay, sort);
        ...
        response.put("bankNiftyATM", atmStrike);
        response.put("bankNiftySpot", niftySpot.getUpdatedAtSource().replace(inputDay, "").trim() +": "+ niftySpot.getLastPrice());
...
//        System.out.println("response: "+response);
    ...
    private Long getDataAtInsertedTime(List<BankNifty> niftyList, String insertedTime, String inputDay) {
        BankNifty nifty = niftyList.stream().filter(n -> n.getUpdatedAtSource().replace(inputDay,"").trim().equals(insertedTime)).findFirst().orElse(null);
